...
class SearchView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        matching_name = list(Restaurant.objects.filter(name__icontains=search_query).values_list('id', flat=True))
        matching_addr = list(Restaurant.objects.filter(address__icontains=search_query).values_list('id', flat=True))
        matching_menu = []

        # Searching for Restaurants with matching menu items
        matching_menu_items = MenuItem.objects.filter(name__icontains=search_query)
